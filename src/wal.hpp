...
enum class WalSpaceType : uint8_t {
    L2 = 0,
    IP = 1
};

enum class WalVectorType : uint8_t {
    FLOAT32 = 0,
    FLOAT16 = 1,
    BFLOAT16 = 2
};
...
public:
    ...
    void logAdd(uint32_t docId, const std::vector<float>& vector,
...
private:
    ...
    static std::vector<uint8_t> serializeAddPayload(uint32_t docId,
