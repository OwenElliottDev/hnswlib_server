...
static uint8_t readU8(const uint8_t *data, size_t &off) { return data[off++]; }
...
WalHeader WriteAheadLog::readHeader(FILE *f) {
  ...
  WalHeader h;
  size_t off = 0;
  std::memcpy(&h.magic, buf + off, 4);
  off += 4;
  std::memcpy(&h.version, buf + off, 4);
  off += 4;
  std::memcpy(&h.dimension, buf + off, 4);
  off += 4;
  std::memcpy(&h.M, buf + off, 4);
  off += 4;
  std::memcpy(&h.efConstruction, buf + off, 4);
  off += 4;
...
std::vector<uint8_t> WriteAheadLog::serializeAddPayload(uint32_t docId, const std::vector<float> &vector,
...
std::pair<WalHeader, std::vector<WalEntry>> WriteAheadLog::readAll(const std::string &path) {
  ...
  while (pos + 4 <= totalSize) {
    uint32_t entryLen;
    if (std::fread(&entryLen, sizeof(entryLen), 1, f) != 1)
      break;
    pos += 4;

    if (pos + entryLen + 4 > static_cast<unsigned long>(totalSize))
      break;

    std::vector<uint8_t> payload(entryLen);
    if (std::fread(payload.data(), 1, entryLen, f) != entryLen)
      break;
    pos += entryLen;

    uint32_t storedCrc;
    if (std::fread(&storedCrc, sizeof(storedCrc), 1, f) != 1)
      break;
    pos += 4;
